import { Block, utils } from "../src/document";
...
test("Sort blocks #1", () => {
    ...
    const sorted = utils.sortBlocks(testBlocks);
...
test("Iterate pairs (normal)", () => {
    ...
    const result = utils.iteratePairs(input, (a, b) => a + b);
...
test("Iterate pairs (single element array)", () => {
    ...
    const result = utils.iteratePairs(input, (a, b) => b);
    expect(result).toEqual(expectedResult);
});

test("Iterate pairs (single element array)", () => {
    const input: Array<string> = ["test"];
    const expectedResult: Array<string> = [];
    const result = utils.iteratePairs(input, (a, b) => b.length);
    expect(result).toEqual(expectedResult);
});
...
test("Mask input and hints #1", () => {
    ...
    expect(utils.maskInputAndHints(inputDocument, blocks)).toEqual(maskedString);
});
...
test("Extract inter-block ranges", () => {
    ...
    const interBlockRanges = utils.extractInterBlockRanges(blocks, document);
