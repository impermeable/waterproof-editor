...
import {
  BUSY_INDICATOR_CLASS,
  BUSY_INDICATOR_DELAY_MS,
  BusyIndicatorMarker,
  CodeBlockBusyIndicator,
} from "../src/codeview/busy-indicator";
...
const cfg: WaterproofEditorConfig = {
  ...
  completions: [],
  documentConstructor: () => [
    new CodeBlock("Qed.", { from: 0, to: 4 }, { from: 0, to: 4 }, 0),
  ],
  symbols: [],
 ...
 */
function makeView(
  indicator: CodeBlockBusyIndicator,
  doc = "line1\nline2\nline3",
): EditorView {
...
describe("CodeBlockBusyIndicator", () => {
  ...
  describe("BusyIndicatorMarker (via toDOM)", () => {
    ...
    test("does not have busy-indicator class before delay elapses", () => {
      const marker = new BusyIndicatorMarker(
        BUSY_INDICATOR_DELAY_MS,
        "test tooltip",
      );
    ...
    test("eq() returns false for a different GutterMarker subclass", () => {
      const a = new BusyIndicatorMarker(BUSY_INDICATOR_DELAY_MS, "a");
      const other = new (class extends GutterMarker {
        eq() {
          return false;
        }
        toDOM() {
