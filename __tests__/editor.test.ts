...
const cfg: WaterproofEditorConfig = {
    ...
    completions: [],
    documentConstructor: () => [new CodeBlock("Hello world.", {from: 0, to: 12}, {from: 0, to: 12}, 0)],
    symbols: [],
...
function diag(startOffset: number, endOffset: number, message = "msg", severity = Severity.Error): OffsetDiagnostic {
...
describe("removeDiagnostic", () => {
    test("returns false when the diagnostic does not exist", () => {
        ...
        const removed = editor.removeDiagnostic(diag(10, 20, "absent", Severity.Warning));
