...
test("Insert code underneath markdown inside input area", () => {
  ...
  const nodeUpdate = new NodeUpdate(config, serializer);
  jest
    .spyOn(serializer, "serializeDocument")
    .mockReturnValue("<input-area># Hello</input-area>");
  const { newTree, result } = nodeUpdate.nodeUpdate(
    step,
    mapping,
    serializer,
...
test("Wrap markdown in hint area", () => {
  ...
  const slice: Slice = new Slice(
    Fragment.from([WaterproofSchema.nodes.hint.create()]),
    0,
    0,
  );
...
test("Wrap markdown in input area", () => {
  ...
  const slice: Slice = new Slice(
    Fragment.from([WaterproofSchema.nodes.input.create()]),
    0,
    0,
  );
  const step = new ReplaceAroundStep(0, 9, 0, 9, slice, 1);
...
function configureNodeMock(deletedString: string) {
  jest
    .spyOn(nodeMock, "resolve")
    .mockReturnValue({ parent: { type: { name: "doc" } } } as ResolvedPos);
...
test("Delete second codeblock in input", () => {
  ...
  const nodeUpdate = new NodeUpdate(config, serializer);
  const { newTree } = nodeUpdate.nodeUpdate(
    step,
    mapping,
    serializer,
    nodeMock,
...
test("Delete a code block between markdown blocks", () => {
  ...
  expect(newTree.root.children[1].contentRange).toStrictEqual({
    from: 5,
    to: 8,
  });
...
test("Delete adjacent code and markdown blocks", () => {
  ...
  const tree = mapping.getMapping();
  const codeNode = tree.root.children.find((node) => node.type === "code");
  const trailingMarkdown = tree.root.children.find(
    (node) => node.type === "markdown" && node.contentRange.from === 14,
  );
  if (!codeNode || !trailingMarkdown)
    throw new Error(
      "Test setup failed: missing code or trailing markdown node",
    );

  const step = new ReplaceStep(
    codeNode.pmRange.from,
    trailingMarkdown.pmRange.to,
    Slice.empty,
  );
  ...
  expect(newTree.root.children[0].type).toBe("markdown");
  expect(newTree.root.children[0].contentRange).toStrictEqual({
    from: 0,
    to: 2,
  });
...
test("Complex deletion undo", () => {
  ...
  const hint = WaterproofSchema.nodes.hint.create(
    { title: "💡 Hint" },
    Fragment.from([
...
test("Undo deletion of first codeblock (without newline)", () => {
  ...
  jest
    .spyOn(serializer, "serializeDocument")
    .mockReturnValue("Hello\n```coq\nCode\n# Hello```");
  nodeUpdate.nodeUpdate(deleteStep, mapping, serializer, nodeMock);
...
test("Wrap code cell in multilean container shifts lineStart", () => {
  ...
  const wrapSlice = new Slice(
    Fragment.from([
      WaterproofSchema.nodes.container.create({ name: "multilean" }),
    ]),
    0,
    0,
  );
...
test("Lift container: code block after container gets lineStart corrected", () => {
  ...
  const newlineBlock = new NewlineBlock(
    { from: 37, to: 38 },
    { from: 37, to: 38 },
    0,
  );
...
test("Wrapping a hint (with tree-children) in a container does not corrupt the mapping tree", () => {
  ...
  const hintNodeBefore = treeBeforeWrap.root.children.find(
    (n) => n.type === "hint",
  );
  ...
  expect(hint.children[0].contentRange.from).toBe(
    containerOpenLen + hintOpenTag.length,
  );
});
...
test("regression test: replaceAroundReplace – wrapping a node inside a container updates container close offsets", () => {
  ...
  const containerBlock = new ContainerBlock(
    ...
    {
      from: 0,
      to: containerOpen.length + mdContent.length + containerClose.length,
    },
    ...
    [
      new MarkdownBlock(
        mdContent,
        {
          ...
          to: containerOpen.length + mdContent.length,
        },
        {
          from: containerOpen.length,
          to: containerOpen.length + mdContent.length,
  ...
  const wrapStep = new ReplaceAroundStep(
    markdownNode.pmRange.from,
    markdownNode.pmRange.to,
    markdownNode.pmRange.from,
    markdownNode.pmRange.to,
    wrapSlice,
    1,
...
test("regression test: replaceAroundDelete – unwrapping a hint inside a container updates container close offsets", () => {
  ...
  expect(updatedContainer.tagRange.to).toBe(
    containerOpen.length + mdContent.length + containerClose.length,
  ); // 16
  ...
  expect(result).toStrictEqual<WrappingDocChange>({
    firstEdit: {
      finalText: "",
      startInFile: hintTagFrom,
      endInFile: hintContentFrom,
    },
    secondEdit: {
      finalText: "",
      startInFile: hintContentTo,
      endInFile: hintTagTo,
    },
  });
});
...
test("regression test: replaceDelete does not include a node whose opening tag lies outside the step range", () => {
  ...
  expect(() =>
    nodeUpdate.nodeUpdate(step, mapping, serializer, nodeMock),
  ).toThrow(NodeUpdateError);
});
