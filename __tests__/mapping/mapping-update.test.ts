...
test("Mapping.update text insert inside input shifts wrapper and later blocks", () => {
  ...
  const updatedInput = updatedTree.root.children.find(
    (node) => node.type === "input",
  );
  const updatedAfter = updatedTree.root.children
    .filter((node) => node.type === "markdown")
    .at(-1);
  ...
  const slice = new Slice(
    Fragment.from([
      WaterproofSchema.nodes.code.create(
        null,
        Fragment.from(WaterproofSchema.text("New")),
      ),
...
test("Mapping.update node insert in the middle shifts lineStart of later code blocks", () => {
  ...
  const slice = new Slice(
    Fragment.from([
      WaterproofSchema.nodes.code.create(
        null,
        Fragment.from(WaterproofSchema.text("Middle")),
      ),
      WaterproofSchema.nodes.newline.create(),
    ]),
    0,
...
test("Regression: deleting the first code block at position 0 routes to nodeUpdate", () => {
  ...
  if (!newlineAfterFirst)
    throw new Error("Test setup: newline after first code block not found");
...
test("Regression: typing inside a code block after undo of its deletion does not throw", () => {
  ...
  const typeStep = new ReplaceStep(
    2,
    2,
    new Slice(Fragment.from(WaterproofSchema.text("X")), 0, 0),
  );
