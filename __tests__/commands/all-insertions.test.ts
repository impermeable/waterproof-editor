...
const initialStateCode = {
  doc: {
    type: "doc",
    content: [{ type: "code", content: [{ type: "text", text: "Content." }] }],
  },
...
const insertableTypes: Array<
  [string, (place: InsertionPlace, conf: TagConfiguration) => Command, string]
> = [
    ...
    for (const place of places) {
      ...
      test(testName, () => {
        ...
        const view = new EditorView(null, {
          state: EditorState.fromJSON({ schema: WaterproofSchema }, cell[1]),
        });
