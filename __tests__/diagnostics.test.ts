...

{
  ...
  test("Add and remove", () => {
    ...
    expect(beforeRemove).toStrictEqual([
      { start: 0, end: 10, message: "test", severity: 1 },
    ]);

    const retVal = editor.removeDiagnostic({
      startOffset: 0,
      endOffset: 10,
      message: "test",
      severity: 1,
  ...
  test("Filter on levels", () => {
    ...
    const truncateAtWarning = editor.getPartialDiagnosticsInRange(
      0,
      10,
      Severity.Warning,
    );
    const truncateAtInfo = editor.getDiagnosticsInRange(
      0,
      10,
      Severity.Information,
    );
    const truncateAtHint = editor.getPartialDiagnosticsInRange(
      0,
      10,
      Severity.Hint,
    );
