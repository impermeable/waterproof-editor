...
describe("startSpinner / stopSpinner", () => {
    test("startSpinner adds the spinner class", () => {
        const { bar, root } = makeBar();
        bar.startSpinner();
        expect(root.querySelector(".spinner-container")!.classList.contains("spinner")).toBe(true);
    });
    ...
    test("stopSpinner removes the spinner class", () => {
        ...
        bar.startSpinner();
        bar.stopSpinner();
        expect(root.querySelector(".spinner-container")!.classList.contains("spinner")).toBe(false);
    });
