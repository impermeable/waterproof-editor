...
import { LocalSemanticToken } from "./codeview/semantic-highlighting";
...
import {
  DOCUMENT_PROGRESS_DECORATOR_KEY,
  documentProgressDecoratorPlugin,
} from "./documentProgressDecorator";
...
export class WaterproofEditor {
  ...
  private updateDocumentProgress() {
    ...
    const activeNodeViews = CODE_PLUGIN_KEY.getState(
      this._view.state
    )?.activeNodeViews;
    // Use getState with the PROGRESS_PLUGIN_KEY to obtain progress status
    const progressParams = PROGRESS_PLUGIN_KEY.getState(
      this._view.state
    )?.progressParams;
    if (
      progressParams === undefined ||
      lineNumbers === undefined ||
      activeNodeViews === undefined
    )
      return;
    ...
    if (
      currentNodeView === undefined ||
      viewLineNumber === undefined ||
    ...
    const endDocCoords = this._view.coordsAtPos(
    ...
    const tr = this._view.state.tr.setMeta(DOCUMENT_PROGRESS_DECORATOR_KEY, {
      total: endDocCoords.top - startDocCoords.top,
      progressHeightLow: height,
      progressHeightHigh: nextCoords.top - startDocCoords.top,
    });
    this._view.dispatch(tr);
  }
  ...
  /**
   * Sets semantic tokens from the LSP for the current document.
   * Follows the same offset-based pattern as {@link setActiveDiagnostics}.
   *
   * @param tokens Array of offset-based semantic tokens.
   */
  public setSemanticTokens(tokens: OffsetSemanticToken[]): void {
    const map = this._mapping;
    if (map === undefined || this._view === undefined) return;
    ...
    // Build an array of { view, pmStart, pmEnd } for each code block
    const blocks = Array.from(nodeViews)
      .map((view) => {
        const pos = view._getPos();
        const cmLength = view["_codemirror"]?.state.doc.length ?? 0;
        return {
          view,
          pmStart: pos,
          pmEnd: pos !== undefined ? pos + 1 + cmLength : undefined,
        };
      })
      .filter(
        (b): b is { view: CodeBlockView; pmStart: number; pmEnd: number } =>
          b.pmStart !== undefined && b.pmEnd !== undefined
      );

    // Pre-allocate a bucket of local tokens for each code block
    const tokensByBlock = new Map<CodeBlockView, LocalSemanticToken[]>();
    for (const block of blocks) {
      tokensByBlock.set(block.view, []);
    }
    ...
    for (const token of tokens) {
      ...
      let pmEnd: number;
      try {
        pmStart = map.textOffsetToPmIndex(token.startOffset);
        pmEnd = map.textOffsetToPmIndex(token.endOffset);
      } catch {
        // Skip tokens that can't be mapped (e.g. in non-code regions)
        continue;
      }

      // Find which code block this token belongs to
      for (const block of blocks) {
        if (pmStart >= block.pmStart && pmEnd <= block.pmEnd) {
          const localFrom = pmStart - block.pmStart - 1;
          const localTo = pmEnd - block.pmStart - 1;
          tokensByBlock.get(block.view)!.push({
            from: localFrom,
            to: localTo,
    ...
    // Dispatch tokens to each code block
    for (const [view, localTokens] of tokensByBlock) {
      view.updateSemanticTokens(localTokens);
    }
  }

  /**
   * Clears all semantic tokens from all code blocks.
   * Call this when the LSP server disconnects or becomes unavailable.
   */
  public clearSemanticTokens(): void {
    if (this._view === undefined) return;
    const views = CODE_PLUGIN_KEY.getState(this._view.state)?.activeNodeViews;
    if (views === undefined) return;
    for (const view of views) view.clearSemanticTokens();
  }
