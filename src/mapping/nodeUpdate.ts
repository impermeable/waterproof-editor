...
export class NodeUpdate {
  ...
  constructor(
    private readonly tagConf: TagConfiguration,
    ...
    switch (type) {
      ...
      case OperationType.replace:
        throw new NodeUpdateError(
          " We do not support ReplaceSteps that replace nodes with other nodes (textual replaces are handled in the textUpdate module) ",
        );
    }
  }
  ...
  doReplaceAroundStep(step: ReplaceAroundStep, mapping: Mapping): ParsedStep {
    ...
    switch (type) {
      case OperationType.insert:
        throw new NodeUpdateError(
          " ReplaceAroundSteps with 'insert' operation type are not supported ",
        );
      case OperationType.delete:
  ...
  replaceInsert(
    ...
    if (!nodeInTree)
      throw new NodeUpdateError(
        " Could not find position to insert node in mapping ",
      );
    const parent = tree.findParent(nodeInTree);
    if (!parent)
      throw new NodeUpdateError(
        " Could not find parent of insertion position in mapping ",
      );
    ...
    const documentPos = atZero
      ? 0
      : useTo
        ? nodeInTree.tagRange.to
        : nodeInTree.tagRange.from;

    let offsetProse = atZero
      ? 0
      : useTo
        ? nodeInTree.pmRange.to
        : nodeInTree.pmRange.from;
    ...
    step.slice.content.forEach((node, _, idx) => {
      ...
      const func = makeNeighbors(
        parentContent.maybeChild(idx - 1),
        parentContent.maybeChild(idx - 2),
        parentContent.maybeChild(idx + 1),
        parentContent.maybeChild(idx + 2),
      );
      const output = this.serializer.serializeNode(node, parent.type, func);
      serialized += output;
      const builtNode = this.buildTreeFromNode(
        node,
        offsetOriginal,
        offsetProse,
        lineCounter,
    ...
    tree.traverseDepthFirst((thisNode: TreeNode) => {
      ...
      if (
        thisNode.pmRange.from < step.from &&
        thisNode.pmRange.to > step.from
      ) {
  ...
  buildTreeFromNode(
    ...
    const lineStart =
      node.type.name === "code" || node.type.name === "math_display"
        ? contentLineStart
        : 0;
    ...
    node.forEach((child, _, idx) => {
      const func = makeNeighbors(
        node.maybeChild(idx - 1),
        node.maybeChild(idx - 2),
        node.maybeChild(idx + 1),
        node.maybeChild(idx + 2),
      );
      const serializedChild = this.serializer.serializeNode(
        child,
        node.type.name,
        func,
      );
      ...
      if (child.type !== WaterproofSchema.nodes.text) {
        const childTreeNode = this.buildTreeFromNode(
          child,
          childOffsetOriginal,
          childOffsetProse,
          childLine,
  ...
  replaceDelete(
    ...
    tree.traverseDepthFirst((node: TreeNode) => {
      if (
        node !== tree.root &&
        node.pmRange.from >= step.from &&
        node.pmRange.to <= step.to
      ) {
    ...
    if (nodesToDelete.length == 0) {
      throw new NodeUpdateError(
        "Could not find any nodes to delete in the given step.",
      );
    }
  ...
  replaceAroundDelete(step: ReplaceAroundStep, tree: Tree): ParsedStep {
    ...
    if (!firstNodeBeingUnwrapped || !lastNodeBeingUnwrapped) {
      throw new NodeUpdateError(
        " Could not find first or last node to unwrap in mapping ",
      );
    }

    // Get all nodes in the range (these are the nodes that will be unwrapped)
    const nodesInRange = tree.nodesInProseRange(
      firstNodeBeingUnwrapped.pmRange.from,
      lastNodeBeingUnwrapped.pmRange.to,
    );
    ...
    if (!wrapperNode)
      throw new NodeUpdateError(
        " Could not find parent of nodes being unwrapped ",
      );
    ...
    if (!wrapperParent)
      throw new NodeUpdateError(" Could not find parent of wrapper node ");
    ...
    const removedNewlines =
      countNewlines(wrappedOpenTag) + countNewlines(wrappedCloseTag);
    ...
    tree.traverseDepthFirst((thisNode: TreeNode) => {
      if (thisNode === tree.root) return;
      if (
        thisNode.pmRange.from < wrapperNode.pmRange.from &&
        thisNode.pmRange.to > wrapperNode.pmRange.to
      ) {
  ...
  replaceAroundReplace(step: ReplaceAroundStep, tree: Tree): ParsedStep {
    ...
    if (!wrappingNode) {
      throw new NodeUpdateError(
        " ReplaceAroundStep replace has no wrapping node ",
      );
    }

    const pmSize = step.slice.size;
    if (pmSize != 2)
      throw new NodeUpdateError(" Size of the slice is not equal to 2 ");

    if (step.slice.content.childCount != 1) {
      throw new NodeUpdateError(
        " We only support ReplaceAroundSteps with a single wrapping node ",
      );
    }
    ...
    if (!nodesBeingWrappedStart || !nodesBeingWrappedEnd)
      throw new NodeUpdateError(" Could not find node in mapping ");
    ...
    if (!parent)
      throw new NodeUpdateError(
        " Could not find parent of nodes being wrapped ",
      );

    const nodesInRange = tree.nodesInProseRange(
      positions.proseStart,
      positions.proseEnd,
    );
    ...
    tree.traverseDepthFirst((thisNode: TreeNode) => {
      if (thisNode === tree.root) return;
      if (
        thisNode.pmRange.from < positions.proseStart &&
        thisNode.pmRange.to > positions.proseEnd
      ) {
