...
export const DOCUMENT_PROGRESS_DECORATOR_KEY =
  new PluginKey<IDocumentProgressDecoratorState>(
    "prosemirror-document-progress-decorator",
  );
...
const DocumentProgressDecoratorPluginSpec: PluginSpec<IDocumentProgressDecoratorState> =
  {
    ...
    view(editorView) {
      ...
      const updateDecorator = (state: IDocumentProgressDecoratorState) => {
        decoratorContainer.style.background = `linear-gradient(to bottom, var(--wp-buttonBackground) 0px, var(--wp-buttonBackground) ${state.progressHeightLow}px, var(--wp-textCodeBlockBackground) ${state.progressHeightHigh}px, var(--wp-editorBackground) ${state.progressHeightHigh + 30}px)`;
        decoratorContainer.style.height = `${state.total}px`;
      };

      // Initialize with current state
      const initialState = DOCUMENT_PROGRESS_DECORATOR_KEY.getState(
        editorView.state,
      );
      if (initialState) {
      ...
      return {
        update(view, _prevState) {
          ...
          const currentState = DOCUMENT_PROGRESS_DECORATOR_KEY.getState(
            view.state,
          );
          if (currentState) {
...
export const documentProgressDecoratorPlugin = new Plugin(
  DocumentProgressDecoratorPluginSpec,
);
