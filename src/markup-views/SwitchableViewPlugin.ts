...
export const SWITCHABLE_VIEW_PLUGIN_KEY =
  new PluginKey<ISwitchableViewPluginState>("prosemirror-realtime-markdown");
...
export function createRealMarkdownView(editorConfig: WaterproofEditorConfig) {
  return (
    ...
    if (!pluginState) {
      throw new Error("no realtime markdown plugin!");
    }
    const nodeViews = pluginState.activeNodeViews;

    // set up NodeView
    const nodeView = new SwitchableView(
      getPos,
      view,
      node.textContent,
      node,
      SWITCHABLE_VIEW_PLUGIN_KEY,
      editorConfig.markdownName ?? "markdown",
      editorConfig.toMarkdown ?? ((input) => toMathInline(input)),
...
const RealMarkdownPluginSpec = (
  editorConfig: WaterproofEditorConfig,
): PluginSpec<ISwitchableViewPluginState> => {
  return {
    ...
    state: {
      ...
      apply(tr, value, _oldState, _newState) {
        ...
        if (tr.getMeta(SWITCHABLE_VIEW_PLUGIN_KEY))
          newCur = tr.getMeta(SWITCHABLE_VIEW_PLUGIN_KEY);
...
export const switchableViewPlugin = (editorConfig: WaterproofEditorConfig) =>
  new ProsePlugin(RealMarkdownPluginSpec(editorConfig));
