...
export function makeNeighbors(
  ...
  return (skipNewlines) => {
    ...
    if (nodeAbove === "newline" && skipNewlines)
      nodeAbove = twoAbove?.type.name ?? null;
    if (nodeBelow === "newline" && skipNewlines)
      nodeBelow = twoBelow?.type.name ?? null;
  ...
  public serializeNode(
    ...
    switch (node.type) {
      case WaterproofSchema.nodes.markdown:
        return this.serializeMarkdown(node, parent, neighbors);
      case WaterproofSchema.nodes.code:
        return this.serializeCode(node, parent, neighbors);
      case WaterproofSchema.nodes.math_display:
        return this.serializeMath(node, parent, neighbors);
      ...
      case WaterproofSchema.nodes.newline:
        return this.serializeNewline();
      default:
        throw new SerializationError(
          `[SerializeNode] Node of type "${node.type.name}" not supported.`,
        );
    }
  }
  ...
  public serializeFragment(fragment: Fragment, parent: string | null): string {
    ...
    fragment.forEach((child, _, idx) => {
      const func = makeNeighbors(
        fragment.maybeChild(idx - 1),
        fragment.maybeChild(idx - 2),
        fragment.maybeChild(idx + 1),
        fragment.maybeChild(idx + 2),
      );
...
export class DefaultTagSerializer extends DocumentSerializer {
  ...
  serializeCode(node: Node): string {
    return (
      this.tagConf.code.openTag + node.textContent + this.tagConf.code.closeTag
    );
  }

  serializeMath(node: Node): string {
    return (
      this.tagConf.math.openTag + node.textContent + this.tagConf.math.closeTag
    );
  }

  serializeMarkdown(node: Node): string {
    return (
      this.tagConf.markdown.openTag +
      node.textContent +
      this.tagConf.markdown.closeTag
    );
  }
  ...
  serializeInput(node: Node): string {
    ...
    node.forEach((child) => {
      const output = this.serializeNode(child, "input", () => {
        return { nodeAbove: null, nodeBelow: null };
      });
      textContent.push(output);
    });
    return (
      this.tagConf.input.openTag +
      textContent.join("") +
      this.tagConf.input.closeTag
    );
  }
  ...
  serializeHint(node: Node): string {
    ...
    const textContent: string[] = [];
    node.forEach((child) => {
      const output = this.serializeNode(child, "hint", () => {
        return { nodeAbove: null, nodeBelow: null };
      });
      textContent.push(output);
    });
