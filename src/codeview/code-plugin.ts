...
import { LanguageConfiguration, ThemeStyle, WaterproofCompletionConfig, WaterproofSymbol } from "../api";
 ...
 */
export function createCoqCodeView(completions: Array<Completion>, beforeRegex: RegExp, symbols: Array<Completion>, editorInstance: WaterproofEditor, initialThemeStyle: ThemeStyle, languageConfig?: LanguageConfiguration){
	return (node: ProseNode, view: EditorView, getPos: () => number | undefined): CodeBlockView => {
		...
		*/
		...
		// set up NodeView
		const nodeView = new CodeBlockView(node, view, editorInstance, getPos, pluginState.schema, completions, beforeRegex, symbols, initialThemeStyle, languageConfig);
...
const CoqCodePluginSpec = (completions: Array<Completion>, beforeRegex: RegExp, symbols: Array<Completion>, editorInstance: WaterproofEditor, initialThemeStyle: ThemeStyle, languageConfig?: LanguageConfiguration) : PluginSpec<ICodePluginState> => { return {
	key: CODE_PLUGIN_KEY,
	state: {
	...
	props: {
		nodeViews: {
			"code" : createCoqCodeView(completions, beforeRegex, symbols, editorInstance, initialThemeStyle, languageConfig)
		}
	}
}};


export const codePlugin = (completionObj: WaterproofCompletionConfig, symbols: Array<WaterproofSymbol>, editorInstance: WaterproofEditor, initialThemeStyle: ThemeStyle, languageConfig?: LanguageConfiguration) => {
	// Here we turn the waterproof completions into proper codemirror completions
	//   with template 'holes'
	const cmCompletions =  completionObj.completions.map((value) => {
		return snippetCompletion(value.template, value);
	});
	const regex = RegExp(completionObj.beforeRegex);
	return new ProsePlugin(CoqCodePluginSpec(cmCompletions, regex, symbols, editorInstance, initialThemeStyle, languageConfig));
};
