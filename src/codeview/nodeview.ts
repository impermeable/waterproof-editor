...
export class CodeBlockView extends EmbeddedCodeMirrorEditor {
  ...
  constructor(
    ...
    const tacticCompletionSource: CompletionSource = function (
    ...
    const symbolCompletionSource: CompletionSource = (
      context: CompletionContext
    ) => {
    ...
    this._codemirror = new CodeMirror({
      ...
      extensions: [
        ...
        linter(this.lintingFunction, {
          ...
          needsRefresh: (() =>
            this.lastUsedDiagnosticsVersion <
            this.editorInstance.diagnosticsVersion).bind(this),
          autoPanel: inInputArea, // Only enable auto panel when this view is inside of an input area
          tooltipFilter: inInputArea
            ? () => {
                return [];
              }
            : undefined, // Don't show tooltips inside of input-areas
          delay: 500,
        }),
        ...optional,
        ...
        this._semanticTokenCompartment.of([
          semanticHighlighting(),
          semanticTokenTheme(initialThemeStyle),
        ]),
        ...
        snippetKeymap.of([
          ...
          },
          {
            key: "Escape",
            run: (target) =>
              execCmdIfInCompletionContext(target, closeCompletion),
          },
          {
            key: "Shift-Tab",
      ...
      dispatch(tr, view) {
        ...
        if (!tr.docChanged) {
          ...
          const teacher = INPUT_AREA_PLUGIN_KEY.getState(
            outerView.state
          )?.teacher;
  ...
  public updateThemeFromVSCode(theme: ThemeStyle, lang: string): void {
    ...
    const lanSyntaxHighlighting =
      lang === "lean4" ? verboseSyntaxHighlighting : coqSyntaxHighlighting;
    this._codemirror?.dispatch({
      effects: [
        this._themeCompartment.reconfigure(lanSyntaxHighlighting(theme)),
        this._semanticTokenCompartment.reconfigure([
          semanticHighlighting(),
          semanticTokenTheme(theme),
        ]),
      ],
    });
  }
  ...
  /**
   * Update the semantic token decorations in this code block.
   * @param tokens Tokens with offsets local to this CodeMirror instance (0-based).
   */
  public updateSemanticTokens(tokens: LocalSemanticToken[]): void {
    this._codemirror?.dispatch({
      effects: setSemanticTokens.of(tokens),
    });
  }

  /** Clear all semantic token decorations from this code block. */
  public clearSemanticTokens(): void {
    this._codemirror?.dispatch({
      effects: clearSemanticTokens.of(undefined),
    });
  }
  ...
  private updateLanguage(lang: string) {
    ...
    this._codemirror?.dispatch({
      effects: this._languageCompartment.reconfigure(lan()),
    });
  }
   ...
   * codemirror view.
   */
  private lintingFunction: LintSource = (
    _view: CodeMirror
  ): readonly Diagnostic[] => {
    ...
    const diags = this.editorInstance
      .getPartialDiagnosticsInRange(
        startPos,
        startPos + _view.state.doc.length + 1
      )
      .map((d) => {
        ...
        return this.preprocessDiagnostic(
          Math.max(d.start - startPos - 1, 0),
...
function execCmdIfInCompletionContext(
  target: CodeMirror,
  cmd: Command
): boolean {
  const status = completionStatus(target.state);
