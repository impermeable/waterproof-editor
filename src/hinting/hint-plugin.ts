    ...
    state: {
      ...
      apply(
        ...
        return tr.docChanged
          ? getHintDecorations(newState, hintNodeType)
          : value;
      },
    },
    props: {
 ...
 */
function getHintDecorations(
  state: EditorState,
  hintNodeType: NodeType,
): DecorationSet {
  // Use the `findDescendantsWithType` helper to get all nodes of type `hintNodeType` in the editor.
  const hints = findDescendantsWithType(state.doc, true, hintNodeType).filter(
    (result) => result.node.content.size > 0,
  );
 ...
 * @returns
 */
function createCollapseDOM(
  view: EditorView,
  hint: { node: PNode; pos: number },
) {
